...
public class MainFrame extends JFrame {
	...
	static KontoPanel kp1 = null;
	static KontoPanel kp2 = null;
	...
	public static void setKonten(int nr1, int nr2) {
	...
	public MainFrame() {
		...
		JSplitPane rechtesPane1 = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,kasse_kunde,kontenPanel);
		rechtesPane1.setDividerLocation(dim.width/2);

		JSplitPane inDieMitte = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,aktionsPanel,rechtesPane1);
	...
	private JScrollPane createAktionsPanel() {
		Vector<BuchungsFabrik> fabriken = createBuchungsFabriken();
	...
	private JPanel createZuruecksetzenKleinerGroesserPanel() {
		...
		JButton fontKleiner = new JButton(new MainFrameResize(true,this));
		JButton fontGroesser = new JButton(new MainFrameResize(false,this));
	...
	private JPanel createKasseKundePanel() {
		...
		kp1 = new KontoPanel(new BewegungRenderer());
		kp1.setKonto(DB.getKonto(KontenName.Kasse.getNr()));
		kp1.init();
		kontenPanel.add(kp1);
		kp2 = new KontoPanel(new BewegungRenderer());
	...
	private Vector<BuchungsFabrik> createBuchungsFabriken() {
		Vector<BuchungsFabrik> fabriken = new Vector<BuchungsFabrik>();
		...
		newSaldoBuchungsFabrik(fabriken ,
				...
				KontenName.RueckstellungenKasse, KontenName.ForderungKasse,false);
	...
	private void newSaldoBuchungsFabrik(Vector<BuchungsFabrik> fabriken ,BuchungsBeschreibung b,KontenName n1,KontenName n2,boolean sollSaldo) {
		SaldoBuchungsFabrik fabrik = new SaldoBuchungsFabrik(n1.getNr(),n2.getNr());
		fabrik.setSollSaldo(sollSaldo);
		fabrik.setArt(b.ordinal());
		fabrik.setText(b.getText());
		fabrik.setErlaeuterung(b.getErlaeuterung());
		fabriken.addElement(fabrik);
	}
	
	private void newSaldoBuchungsFabrik(Vector<BuchungsFabrik> fabriken ,BuchungsBeschreibung b,KontenName n1,KontenName n2) {
		newSaldoBuchungsFabrik(fabriken, b, n1, n2,true);
	}
	
	private void newEinfacheBuchungsFabrik(Vector<BuchungsFabrik> fabriken,BuchungsBeschreibung b,KontenName n1,KontenName n2,double betrag) {
		EinfacheBuchungsFabrik fabrik = new EinfacheBuchungsFabrik(n1.getNr(),n2.getNr(),betrag);
		fabrik.setArt(b.ordinal());
		fabrik.setText(b.getText());
		fabrik.setErlaeuterung(b.getErlaeuterung());
